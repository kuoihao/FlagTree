...
struct MulhiUIOpConversion
  ...
  SmallVector<Value> createDestOps(MulhiUIOp op, Adaptor adaptor,
    ...
    if (funcName.empty()) {
      auto intTy = cast<IntegerType>(resultElementTy);
      unsigned bitWidth = intTy.getWidth();
      Type wideTy = IntegerType::get(rewriter.getContext(), bitWidth * 2);
      Value lhsWide =
          LLVM::ZExtOp::create(rewriter, loc, wideTy, operands[0][0]);
      Value rhsWide =
          LLVM::ZExtOp::create(rewriter, loc, wideTy, operands[0][1]);
      Value prodWide =
          LLVM::MulOp::create(rewriter, loc, wideTy, lhsWide, rhsWide);
      auto b = TritonLLVMOpBuilder(loc, rewriter);
      Value shift = b.int_val(bitWidth * 2, bitWidth);
      Value highWide =
          LLVM::LShrOp::create(rewriter, loc, wideTy, prodWide, shift);
      return {LLVM::TruncOp::create(rewriter, loc, resultElementTy, highWide)};
    }
