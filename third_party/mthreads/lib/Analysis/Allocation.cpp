...
private:
  ...
  void resolveLiveness() {
    ...
    auto getValueLivenessRange = [&](Value value) {
      ...
      // SQMMA local_alloc buffers are consumed together by dot instructions
      // inside the loop body; keep them alive through the enclosing for-op so
      // A/B tiles are not assigned overlapping shared-memory regions.
      if (auto localAlloc =
              dyn_cast_or_null<gpu::LocalAllocOp>(value.getDefiningOp())) {
        bool isSqmma = localAlloc->hasAttr("sqmma.op_idx") ||
                       localAlloc->hasAttr("sqmma.opIdx");
        if (isSqmma) {
          if (auto forOp = localAlloc->getParentOfType<scf::ForOp>())
            maxId = std::max(maxId, operationId[forOp.getOperation()] + 1);
        }
      }
  ...
  void buildInterferenceGraph(const SmallVector<BufferT *> &buffers,
                              GraphT &interference) {
    auto isSqmmaBuffer = [](BufferT *buffer) -> bool {
      auto alloc = dyn_cast_or_null<gpu::LocalAllocOp>(buffer->owner);
      if (!alloc || !alloc.isSharedMemoryAlloc())
        return false;
      bool isSqmma =
          alloc->hasAttr("sqmma.op_idx") || alloc->hasAttr("sqmma.opIdx");
      if (!isSqmma)
        return false;
      return true;
    };
    auto getParentForOp = [](BufferT *buffer) -> scf::ForOp {
    ...
    for (auto x : buffers) {
      for (auto y : buffers) {
        ...
        // scoped to a single loop; sequential loop/remainder regions can safely
        // reuse shared memory through normal liveness.
        if (isSqmmaBuffer(x) && isSqmmaBuffer(y) &&
            getParentForOp(x) == getParentForOp(y) && getParentForOp(x) &&
            xSizeRange.intersects(ySizeRange)) {
          interference[x].insert(y);
        }
      }
