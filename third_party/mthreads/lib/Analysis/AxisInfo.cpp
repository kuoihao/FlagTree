...
public:
  ...
  getAxisInfo(arith::ConstantOp op,
    ...
    if (splatAttr && splatAttr.getElementType().isIntOrIndex()) {
      auto shapedTy = dyn_cast<ShapedType>(splatAttr.getType());
      if (!shapedTy || !shapedTy.hasRank())
        return AxisInfo();
      int64_t value = splatAttr.template getSplatValue<APInt>().getZExtValue();
      return AxisInfo(
          /*contiguity=*/AxisInfo::DimVectorT(shapedTy.getRank(), 1),
          /*divisibility=*/
          AxisInfo::DimVectorT(shapedTy.getRank(), highestPowOf2Divisor(value)),
          /*constancy=*/
          AxisInfo::DimVectorT(shapedTy.getShape().begin(),
