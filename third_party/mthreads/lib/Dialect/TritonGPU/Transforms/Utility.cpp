...
rebuildPassthroughWaitLikeOp(OpBuilder &builder, Operation *op,
                             ValueRange newOperands) {
  auto typeInfer = dyn_cast<InferTypeOpInterface>(op);
  if (!typeInfer || op->getNumRegions() != 0 || op->getNumSuccessors() != 0)
    return failure();

  SmallVector<Type> newTypes;
  if (failed(typeInfer.inferReturnTypes(
          op->getContext(), op->getLoc(), newOperands, op->getAttrDictionary(),
          op->getPropertiesStorage(), op->getRegions(), newTypes)))
    return failure();

  OperationState state(op->getLoc(), op->getName(), newOperands, newTypes,
                       op->getAttrs());
...
void replaceUsesAndPropagateType(
  ...
  for (OpOperand *operand : operandsToReplace) {
    if (isPassthroughWaitLikeOp(operand->getOwner())) {
      Operation *wait = operand->getOwner();
      builder.setInsertionPointAfter(wait);
      auto operands = llvm::to_vector(wait->getOperands());
      operands[operand->getOperandNumber()] = val;
      FailureOr<Operation *> newWait =
          rebuildPassthroughWaitLikeOp(builder, wait, operands);
      if (failed(newWait))
        llvm::report_fatal_error("failed to rebuild passthrough wait-like op "
