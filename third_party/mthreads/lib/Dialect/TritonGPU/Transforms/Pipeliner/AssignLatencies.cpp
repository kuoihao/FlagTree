...
static bool supportsGenericDotLoadPipelining(Operation *op) {
  if (auto disableAttr =
          op->getAttrOfType<BoolAttr>(kDisableGenericDotPipelineAttr);
      disableAttr && disableAttr.getValue())
    return false;
  auto dotOp = dyn_cast<tt::DotOpInterface>(op);
  if (!dotOp)
    return false;
  return isa<RankedTensorType>(dotOp.getA().getType()) &&
         isa<RankedTensorType>(dotOp.getB().getType());
}
...
loadOpsToIndirectionLevel(scf::ForOp forOp, bool pipelineWithoutDot,
  ...
  std::function<void(Operation *, Operation *, int)> dfs =
      [&](Operation *op, Operation *finalUser, int distance) {
        ...
        for (Value operand : getNestedOperands(op)) {
          if (supportsGenericDotLoadPipelining(op)) {
  ...
  for (Operation &op : forOp.getBody()->without_terminator()) {
    ...
    if (!supportsGenericDotLoadPipelining(&op) && !isa<ttng::TMEMStoreOp>(op))
      continue;
    seenDot = true;
    seen.clear();
    dfs(&op, &op, 0);
