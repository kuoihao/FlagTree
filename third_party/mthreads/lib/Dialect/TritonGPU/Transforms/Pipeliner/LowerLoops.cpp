...
getMusaSqmmaPipelinedSharedEncoding(RankedTensorType tensorTy) {
  auto cgaLayout = ttg::getCGALayout(tensorTy.getEncoding());
  auto order = ttg::getOrder(tensorTy);
  auto fallback = [&]() -> ttg::SharedEncodingTrait {
    return ttg::SwizzledSharedEncodingAttr::get(tensorTy.getContext(), 1, 1, 1,
                                                order, cgaLayout);
  };
  if (order.empty())
    return fallback();

  int elemBitWidth = tensorTy.getElementTypeBitWidth();
  if (elemBitWidth <= 0)
    return fallback();

  auto shapePerCTA =
      ttg::getShapePerCTA(cgaLayout.getCTASplitNum(), tensorTy.getShape());
  if (order[0] >= shapePerCTA.size())
    return fallback();

  int64_t contigDimSizeInBytes = shapePerCTA[order[0]] * elemBitWidth / 8;
  unsigned perPhase = 1;
  unsigned maxPhase = 1;
  if (contigDimSizeInBytes >= 128 && contigDimSizeInBytes % 128 == 0) {
    perPhase = 1;
    maxPhase = 8;
  } else if (contigDimSizeInBytes >= 64 && contigDimSizeInBytes % 64 == 0) {
    perPhase = 2;
    maxPhase = 4;
  } else if (contigDimSizeInBytes >= 32 && contigDimSizeInBytes % 32 == 0) {
    perPhase = 4;
    maxPhase = 2;
  } else if (contigDimSizeInBytes >= 16 && contigDimSizeInBytes % 16 == 0) {
    perPhase = 8;
    maxPhase = 1;
  }

  unsigned vec = std::max(1u, 128u / static_cast<unsigned>(elemBitWidth));
  return ttg::SwizzledSharedEncodingAttr::get(
      tensorTy.getContext(), vec, perPhase, maxPhase, order, cgaLayout);
}
    ...
    if (isa<tt::LoadOp, tt::DescriptorLoadOp, tt::DescriptorGatherOp>(op)) {
      ...
      if (canUseAsyncCp || isTMALoad(&op)) {
        ...
        if (isMusaTarget && isa<tt::LoadOp>(op) && hasDotConsumer(&op)) {
          // MUSA SQMMA async-copy path requires one extra ring buffer to match
          // the expected software-pipeline staging depth.
          stageDiff += 1;
          sharedEncoding = getMusaSqmmaPipelinedSharedEncoding(
