...
public:
  ...
  LogicalResult matchAndRewrite(ConvertLayoutOp cvtOp,
    ...
    if (!trans || trans.getOrder() != ArrayRef<int32_t>{1, 0})
      return failure();
    // Only rewrite when transpose is exclusively consumed by this convert.
    if (!trans->hasOneUse())
      return failure();
...
public:
  ...
  LogicalResult matchAndRewrite(LocalAllocOp allocOp,
    ...
    if (auto dotTy = dyn_cast<RankedTensorType>(dot->getResult(0).getType())) {
      if (isa<MUSASqmmaEncodingAttr, MUSAWmmaEncodingAttr>(dotTy.getEncoding()))
        return failure();
    }
    // Match outerCvt(trans(innerCvt(x))).
