...
Value getExpandedOffsetWithRange(OpBuilder &builder, const Location &loc,
  ...
  assert(offsetType && "expected integer offset type");
  unsigned width = offsetType.getWidth();
  auto indexElemType = builder.getIntegerType(width);
  // Add range.
  auto indexI32RowType =
      RankedTensorType::get({blockShape[dim]}, builder.getI32Type());
  auto indexRowType = RankedTensorType::get({blockShape[dim]}, indexElemType);
  Value splatOffset =
      triton::SplatOp::create(builder, loc, indexRowType, offset);
  Value range = triton::MakeRangeOp::create(builder, loc, indexI32RowType, 0,
                                            blockShape[dim]);
  Value typedRange = range;
  if (width > 32) {
    typedRange = arith::ExtSIOp::create(builder, loc, indexRowType, range);
  } else if (width < 32) {
    typedRange = arith::TruncIOp::create(builder, loc, indexRowType, range);
  }

  Value offsets = arith::AddIOp::create(builder, loc, splatOffset, typedRange);
...
Value generatePtrFromOffsetRanges(OpBuilder &builder, Location loc,
  ...
  for (unsigned i = 0; i < blockShape.size(); ++i) {
    ...
    auto indexTensorType = RankedTensorType::get(
        blockShape,
        mlir::cast<ShapedType>(offsetWithStride.getType()).getElementType());
...
Value generateMaskFromOffsetRanges(OpBuilder &builder, const Location &loc,
  ...
  for (std::size_t i = 0; i < blockShape.size(); ++i) {
    ...
    auto offsetElemType = mlir::dyn_cast<mlir::IntegerType>(
        mlir::cast<ShapedType>(offsetWithRange.getType()).getElementType());
    assert(offsetElemType && "expected integer offset tensor type");
    auto offsetWidth = offsetElemType.getWidth();

    // Compare with lower bound
    Value lowerBound =
        mlir::arith::ConstantIntOp::create(builder, loc, 0, offsetWidth);
    ...
    Value upperBound = desc.shape[i];
    if (upperBound.getType() != offsetElemType) {
      auto upperIntTy = mlir::dyn_cast<mlir::IntegerType>(upperBound.getType());
      assert(upperIntTy && "expected integer shape type");
      if (upperIntTy.getWidth() > offsetWidth) {
        upperBound =
            arith::TruncIOp::create(builder, loc, offsetElemType, upperBound);
      } else if (upperIntTy.getWidth() < offsetWidth) {
        upperBound =
            arith::ExtSIOp::create(builder, loc, offsetElemType, upperBound);
      }
    }
...
SmallVector<mlir::Value> castToI32(OpBuilder &builder,
                                   mlir::ValueRange values) {
  auto i32Type = builder.getI32Type();
  return llvm::map_to_vector(values, [&](mlir::Value v) -> mlir::Value {
    auto vType = mlir::dyn_cast<mlir::IntegerType>(v.getType());
    if (!vType)
      return v;
    if (vType == i32Type)
      return v;
    if (vType.getWidth() > 32)
      return builder.createOrFold<arith::TruncIOp>(v.getLoc(), i32Type, v);
    return builder.createOrFold<arith::ExtSIOp>(v.getLoc(), i32Type, v);
  });
}
...
struct RewriteLoadPattern : OpConversionPattern<triton::DescriptorLoadOp> {
  ...
  matchAndRewrite(triton::DescriptorLoadOp op, OneToNOpAdaptor adaptor,
    ...
    bool useI32Path =
        blockShape.size() == 2 &&
        llvm::all_of(op.getIndices().getTypes(), [](mlir::Type ty) {
          auto intTy = dyn_cast<mlir::IntegerType>(ty);
          return intTy && intTy.getWidth() <= 32;
        });
    SmallVector<mlir::Value> offsetsI32;
    SmallVector<mlir::Value> shapeI32;
    SmallVector<mlir::Value> stridesI32;
    Descriptor descI32;
    if (useI32Path) {
      offsetsI32 = castToI32(rewriter, op.getIndices());
      shapeI32 = castToI32(rewriter, desc.shape);
      stridesI32 = castToI32(rewriter, desc.strides);
      descI32 = Descriptor{desc.base, shapeI32, stridesI32, desc.paddingOption};
    }
    auto other = generateOther(rewriter, loc, descTy, desc.paddingOption);
    auto newLoad = rewriter.replaceOpWithNewOp<triton::LoadOp>(
        op,
        useI32Path ? generatePtr(rewriter, loc, blockShape, descI32, offsetsI32)
                   : generatePtr(rewriter, loc, blockShape, desc, offsets),
        useI32Path
            ? generateMask(rewriter, loc, blockShape, descI32, offsetsI32)
            : generateMask(rewriter, loc, blockShape, desc, offsets),
        other, triton::CacheModifier::NONE, triton::EvictionPolicy::NORMAL,
        false);
