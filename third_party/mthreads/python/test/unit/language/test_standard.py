...
def test_sort(M, N, k, descending, dtype_str, device):
    ...
    if k is None or x.numel() < k:
        ...
        try:
            ...
            # MUSA torch.topk currently rejects integral dtypes even though
            # Triton's bitonic top-k path handles them correctly.
            if device == "musa" and "Dtype of input tensor of topk only support" in str(exc):
                y = torch.topk(x.cpu(), k=k).values.to(device)
            else:
                raise
