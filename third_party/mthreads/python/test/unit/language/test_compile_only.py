...
def _has_cuda_sm100_compile_only_backend() -> bool:
    target = GPUTarget("cuda", 100, 32)
    return sum(1 for backend in backends.values() if backend.compiler.supports_target(target)) == 1


requires_cuda_sm100_backend = pytest.mark.skipif(
    not _has_cuda_sm100_compile_only_backend(),
    reason="Requires NVIDIA CUDA backend with SM100 compile-only support",
)


@requires_cuda_sm100_backend
