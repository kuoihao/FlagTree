...
from triton._internal_testing import is_cuda, is_hip, is_hip_cdna2, is_hip_cdna3, is_hip_cdna4, is_hip_rdna4, is_musa, is_musa_ph1
...
def sanitize_fnuz_special_value(x):
    # FNUZ fp8 uses the raw bit-pattern 0x80, which is stored through int8
    # tensors as the signed value -128. MUSA int8 comparisons against the
    # literal 0x80 do not match that storage value, so normalize via the
    # signed representation to keep the emulation inputs stable across
    # backends.
    return torch.where(x == torch.iinfo(x.dtype).min, torch.zeros_like(x), x)
...
def launch_exhaustive_populate(dst_dtype, offset, numel, force_odd, output_bits, max_repr, device, BLOCK_SIZE=4096):
    ...
    if dst_dtype == tl.float8e4b8 or dst_dtype == tl.float8e5b16:
        dst = sanitize_fnuz_special_value(dst)
    return dst
...
def test_typeconvert_downcast(src_dtype, dst_dtype, rounding, max_repr, device):
    ...
    if is_musa():
        if is_musa_ph1() and dst_dtype in ('float8e4b8', 'float8e5b16'):
            pytest.skip(f"{dst_dtype} is not supported on MUSA PH1")
...
def test_typeconvert_downcast_clamping(src_dtype, dst_dtype, mode, device, rounding="rtne"):
    ...
    if mode == 'nan':
        ...
        # MUSA runtime currently cannot materialize float8 tensors via Fill,
        # so build the float8 reference on CPU and compare there instead.
        if is_musa() and torch_dst_dtype in (torch.float8_e4m3fn, torch.float8_e5m2):
            expected = torch.full(dst.shape, expected_result, dtype=torch_dst_dtype)
