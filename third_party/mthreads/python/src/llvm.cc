...
namespace {
void initializeTritonLLVMTargets() {
  // Keep host target registration for host-side LLVM utilities.
  (void)llvm::InitializeNativeTarget();
  (void)llvm::InitializeNativeTargetAsmParser();
  (void)llvm::InitializeNativeTargetAsmPrinter();

#if TRITON_ENABLE_NVIDIA
