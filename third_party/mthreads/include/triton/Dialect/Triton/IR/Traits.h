...
// Marker trait for wait ops that thread selected operands through SSA results
// without changing their types. Utility/layout passes can rebuild these ops
// generically when operand types are rewritten.
template <typename ConcreteType>
struct PassthroughWaitLike
    : public TraitBase<ConcreteType, PassthroughWaitLike> {};
